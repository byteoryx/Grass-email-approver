...
def account_flow(lock: threading.Lock, account_index: int, proxy: str, account: str, config: dict, task: int):
	try:
		...
		if task == 1:
			...
			ok = wrapper(grass_instance.check, 1)
			if ok != 0:
				with lock:
					with open("data/success_data.txt", "a") as f:
						f.write(f"{account}:{proxy}\n")
					append_to_csv("data/result.csv", account, ok)
...
def append_to_csv(file_path, text_value, number_value):
    ...
    with open(file_path, mode='a', newline='', encoding='utf-8') as file:
        ...
        # Если файл новый, можно добавить заголовки (необязательно)
        if not file_exists:
            writer.writerow(["Wallet address", "Tokens"])
        
        # Записываем строку с текстом и числом
        writer.writerow([text_value, number_value])